...
describe('GOLDEN: the live top 10, in order', () => {
  ...
  it('is the order the listing page actually renders', () => {
    // index.astro ranks with sortStudies over live studies that have a known
    // rate. Re-running that sort on the fixture-derived records must match
    // the audited board; a change to the comparator cannot hide behind a
    // passing hand-built list.
    const pageTop10 = sortStudies(
      studies.filter((s) => !s.isExpired && isKnownRate(s.effectiveHourly)),
      'desc',
    ).slice(0, 10);

    expect(pageTop10.map((s) => [s.id, s.effectiveHourly])).toEqual(EXPECTED_LIVE_TOP_10);
  });

  it('renders the unrated block money-first', () => {
    // index.astro builds the second section as
    //   sortUnrated(live.filter((s) => !isKnownRate(s.effectiveHourly)))
    // The head of this block is the only part a reader scans, and #11321 is $1,000.
    const pageUnrated = sortUnrated(
      studies.filter((s) => !s.isExpired && !isKnownRate(s.effectiveHourly)),
    );

    expect(pageUnrated.slice(0, 3).map((s) => [s.id, guaranteedTotal(s)])).toEqual([
      ['11321', 1000],
      ['12775', 400],
      ['8404', 350],
    ]);
    const firstUnpriced = pageUnrated.findIndex((s) => guaranteedTotal(s) === null);
    expect(pageUnrated.slice(firstUnpriced).every((s) => guaranteedTotal(s) === null)).toBe(true);
  });
