...
import type { UserProfile } from '@/studies/types.ts';

type ProfileModule = typeof import('@/studies/profile.ts');
...
async function loadInBrowser(seed: Record<string, string> = {}, storageThrows = false) {
  ...
  vi.resetModules();
  const mod: ProfileModule = await import('@/studies/profile.ts');
