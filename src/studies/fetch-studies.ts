...
import { STUDY_API_URL, TAXONOMY_ENDPOINTS } from '@/studies/types.ts';
import type { RawStudy, TaxonomyMaps, TaxonomyTerm } from '@/studies/types.ts';
...
export const USER_AGENT =
  'radar/1.0 (+https://harsh.bet/radar; focused research + campus + paid-study aggregator; static site, one pass per build)';
...
const TAXONOMY_CACHE_URL = new URL('../data/studies-taxonomies.json', import.meta.url);
  ...
  try {
    ...
    try {
      ...
      if (total > 0) {
        ...
        log.warn(`[fetch:taxonomies] SOURCE=cache - reused ${total} terms from src/data/studies-taxonomies.json`);
